(function (window) {
    ...
    Seed.prototype = {
        ...
        canScale: function () {
            return this.leaf.scale > 0.2;
        },
        setLeafScale: function (scale) {
            this.leaf.scale *= scale;
        },
        scale: function (scale) {
            this.clear();
            this.drawCircle();
            this.drawLeaf();
            this.setLeafScale(scale);
        },
        drawLeaf: function () {
            var ctx = this.tree.ctx, leaf = this.leaf;
            var point = leaf.point, color = leaf.color,
                scale = leaf.scale;
            ctx.save();
            ctx.fillStyle = color;
            ...
            ctx.beginPath();
            ctx.moveTo(0, 0);
            for (var i = 0; i < leaf.figure.length; i++) {
                var p = leaf.figure.get(i, scale);
                ctx.lineTo(p.x, -p.y);
            }
            ctx.closePath();
            ctx.fill();
    ...
    Footer = function (tree, width, height, speed) {
        this.tree = tree;
        this.point = new Point(tree.seed.leaf.point.x, tree.height - height / 2);
        this.width = width;
        this.height = height;
        this.speed = speed || 2;
        this.length = 0;
    ...
    Branch = function (tree, point1, point2, point3, radius, length, branches) {
        this.tree = tree;
        this.point1 = point1;
        this.point2 = point2;
        this.point3 = point3;
